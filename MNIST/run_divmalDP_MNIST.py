        ...
        for key, value in state_dict.items():
            ...
            for i in range(len(deltas)):    
                ...
                if any(i < m for m in malModel) :
                    noise = (np.random.normal((np.sqrt(2*gamma/30)*(sigma*S_value)), float((sigma)*(S_value)/np.sqrt(30)/2), size = deltas[i][key].shape))
                
                else: 
                    noise = (np.random.normal(0, float((sigma)*(S_value)/np.sqrt(30)/2), size = deltas[i][key].shape))                
