        ...
        for key, value in state_dict.items():
            ...
            for i in range(len(deltas)):    
                ...
                if any(i < m for m in malModel) :
                    noise = (np.random.normal((np.sqrt(2*gamma/30)*(sigma*S_value)), float((sigma)*(S_value)/30), size = deltas[i][key].shape))
                
                else: 
                    noise = (np.random.normal(0, float((sigma)*(S_value)/30), size = deltas[i][key].shape))
...
#             noise = np.random.normal(0, float(S_value * sigma), size = suma.shape)
        ...
        for initial_idx in range(mt):
            centroid =  flattened_deltas[initial_idx]
            for iteration in range(max_iterations):
                distances = torch.norm(flattened_deltas - centroid, dim=1)
                sorted_indices = torch.argsort(distances)    
                top_k_indices = sorted_indices[:k]
                top_k_points = flattened_deltas[top_k_indices]
                new_centroid = top_k_points.mean(dim=0)
                shift = torch.norm(new_centroid - centroid)
                if shift < tolerance:
                    break
                centroid = new_centroid
            total_distance = torch.norm(flattened_deltas[top_k_indices] - centroid, dim=1).sum()
            if total_distance < best_total_distance:
                best_total_distance = total_distance
                best_top_k_indices = top_k_indices
                best_centroid = centroid
        count = len(best_top_k_indices[best_top_k_indices<5])
