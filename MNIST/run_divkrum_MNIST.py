        ...
        for key, value in state_dict.items():
            ...
            for i in range(len(deltas)):    
                ...
                if any(i < m for m in malModel) :
                    noise = (np.random.normal((np.sqrt(2*gamma)*(S_value)/30), float((sigma)*(S_value**2)), size = deltas[i][key].shape))

                else: 
                    noise = (np.random.normal(0, float((sigma)*(S_value**2)), size = deltas[i][key].shape))
                #if any(i < m for m in malModel) :
